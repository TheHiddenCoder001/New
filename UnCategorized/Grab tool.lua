...
local Defaults = {
	...
	colors = {BrickColor.new("Bright blue"),BrickColor.new("Medium blue"),BrickColor.new("Teal")},
	highlightcolors = {Anchored = Color3.new(1,0.2,0.2), Unanchored = Color3.new(1,0.5,0.2), Owned = Color3.new(0.3,0.6,1), OwnedJoint = Color3.new(0.5,0.9,1), JointAnchored = Color3.new(1,0.9,0.3)},
	scrollspeed = 1,
	rotspeed = 5,
	GiveOnRespawn = false,
}
...
local function Initiate()
			...
			if lookingat and not enabled and equipped and lookingat.Name ~= "Terrain" then
				...
				if lookingat.Anchored then
					...
					if isnetworkowner(lookingat) then
						...
						local RootPart = lookingat:GetRootPart()
						if RootPart then
							if isnetworkowner(RootPart) then
								selection.Color3 = _G.highlightcolors.OwnedJoint
							elseif RootPart.Anchored then
								selection.Color3 = _G.highlightcolors.JointAnchored
							else
	...
	local function Grab(object)
		...
		local rotvel = Instance.new("BodyAngularVelocity")
		rotvel.MaxTorque = Vector3.new(math.huge,math.huge,math.huge)
		rotvel.AngularVelocity = Vector3.new()
		...
		repeat
			...
			if holding then
				...
				local rotation = Vector3.new(rotation.X,rotation.Y,rotation.Z)
				if rotation.Magnitude > 0.1 then
					rotvel.Parent = holding
					rotvel.AngularVelocity = rotation
				else
					rotvel.AngularVelocity = Vector3.new()
				end
	...
	cons["holdcon"] = mouse.Button1Down:Connect(function()
		if equipped then
			helddown = true
			repeat
				wait()
	...
	cons["endcon"] = mouse.Button1Up:Connect(function()
		enabled = false
		helddown = false
	end)

	-- Keybinds
	cons["inpstartcon"] = uis.InputBegan:Connect(function(inpute, gse)
		if equipped and not gse then
			...
			elseif keycode == Enum.KeyCode.C then
				if lookingat then
					local RootPart = lookingat:GetRootPart()
					if RootPart then
						local primary = Instance.new("SelectionBox")
						primary.Parent = RootPart
						primary.Adornee = RootPart
						primary.LineThickness = 1
						primary.SurfaceTransparency = 0.5
						primary.Color3 = Color3.new(1,1,1)
						primary.SurfaceColor3 = Color3.new(1,1,1)
						tween:Create(primary,TweenInfo.new(1,Enum.EasingStyle.Quad,Enum.EasingDirection.Out),{LineThickness=0,Transparency=1,SurfaceTransparency=1}):Play()
						debris:AddItem(primary, 1)
					end
				end
			elseif keycode == Enum.KeyCode.J then
				if holding then
					holding.CanCollide = not holding.CanCollide
				end
	...
	cons["inpendcon"] = uis.InputEnded:Connect(function(inpute)
		local keycode = inpute.KeyCode
		if keycode == Enum.KeyCode.Q then
			retracting = 0
		elseif keycode== Enum.KeyCode.E then
...
Initiate()
if _G.GiveOnRespawn then
	local chradd = player.CharacterAdded:Connect(function()
		Initiate()
	end)
	plrchat = player.Chatted:Connect(function(chat)
		if chat == "/e nograbtool" then
			chradd:Disconnect()
			plrchat:Disconnect()
		end
	end)
